...
- **Iteration-3 rule:** if the same class of error has been fixed manually three times, write a tool. `Logger(subsystem:category:)` was fixed 15 times during the ADR-0007 refactor — exactly the case the rule was made for.
...
|---|---|---|
| `max-file-loc` | File > 500 non-blank, non-comment lines | Generic file-size signal |
...
A Claude Code PreToolUse hook can fire the linter on Edit/Write of any Swift file in this repo. Violations block the operation with the remediation hint visible to Claude. Hook configuration is environment-specific and lives outside this repo.
...
- A Claude Code PreToolUse hook (configured outside this repo) can fire the linter on Swift edits.
...
- The single-file Swift linter pattern (script with shebang, regex rules, embedded remediation hints) is the prior-art template adapted here.
